...
const categories = [
  { name: "Fruits", desc: "Popular fruits and high-demand listings." },
  { name: "Bundles", desc: "Grouped offers and premium packs." },
  { name: "Accounts", desc: "Player listings and leveled profiles." },
...
export default function BloxFruitsPage() {
  ...
  useEffect(() => {
    const fetchBloxFruitsListings = async () => {
      setLoading(true);
      setErrorMessage("");
      ...
      if (error) {
        setErrorMessage("Could not load Blox Fruits listings.");
        setListings([]);
        setLoading(false);
        return;
      }

      const bloxFruitsListings = (data ?? []) as Listing[];
      setListings(bloxFruitsListings);

      const uniqueUserIds = [...new Set(bloxFruitsListings.map((item) => item.user_id))];
  ...
  return (
    <div className="relative min-h-screen bg-[#0B0B12] text-[#F5F7FF]">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(59,130,246,0.14),transparent_35%),radial-gradient(circle_at_top_right,rgba(34,211,238,0.10),transparent_28%)]" />
                ...
                <div className="w-full rounded-[24px] border border-white/10 bg-black/20 p-5 backdrop-blur">
                  {featuredListing ? (
                    ...
                    <>
                      ...
                      <div className="mt-1 text-sm text-[#9CA3AF]">
                        Create the first Blox Fruits listing on Dxblox.
                      </div>
                      <Link
                        href="/create-listing"
                        className="mt-5 block w-full rounded-2xl bg-white/10 px-4 py-3 text-center font-semibold text-white transition hover:bg-white/15"
        ...
        <section className="rounded-[26px] border border-white/10 bg-[#131320] p-5 mt-6">
          <div className="grid gap-4 lg:grid-cols-6">
            ...
            >
              ...
              <option value="Fruits" className="bg-[#131320] text-white">
                Fruits
              </option>
              <option value="Bundles" className="bg-[#131320] text-white">
                Bundles
              </option>
              <option value="Accounts" className="bg-[#131320] text-white">
                Accounts
              </option>
            ...
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 text-sm text-cyan-300">{count} listings</div>
                </div>
              );
            })}
          </div>
        ...
        <section className="mt-10 grid gap-8 xl:grid-cols-[1fr_320px]">
          <div>
            ...
            {loading ? (
              ...
              <div className="rounded-[24px] border border-white/10 bg-[#131320] p-6 text-[#9CA3AF]">
                No Blox Fruits listings match your filters yet.
              </div>
            ) : (
